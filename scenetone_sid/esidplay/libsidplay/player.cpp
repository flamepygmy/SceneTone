...
EXPORT_C bool sidEmuInitializeSong(emuEngine & thisEmuEngine,
								   ...
								   uword songNumber)
	{
	ubyte* c64mem1 = thisEmuEngine.iThe6510->c64mem1;
	ubyte* c64mem2 = thisEmuEngine.iThe6510->c64mem2;
	...
	if (ret && (thisEmuEngine.config.digiPlayerScans!=0))
	{
		...
		if ( replayPC == 0 )
		{
			thisEmuEngine.playRamRom = thisEmuEngine.iThe6510->c64mem1[1];
			if ((thisEmuEngine.playRamRom & 2) != 0)  // isKernal ?
			{
		...
		while (loops)
		{
			...
			{
				if (thisEmuEngine.oldValues[i] != c64mem2[c64addrTable[i]])
				{
					useDigis = true;
					break;
				}
				thisEmuEngine.oldValues[i] = c64mem2[c64addrTable[i]];
			}
			thisEmuEngine.iThe6510->interpreter(replayPC, thisEmuEngine.playRamRom,0,0,0);
			loops--;
			if (useDigis)
			{
				break;
...
bool sidEmuInitializeSongOld(emuEngine & thisEmuEngine,
	...
	else
	{
		...
		thisEmuEngine.iTheSidEmu->sidEmuSetReplayingSpeed(the_clock,the_speed);
		...
		thisEmuEngine.iThe6510->interpreter(thisTune.info.initAddr, thisEmuEngine.iThe6510->c64memRamRom(thisTune.info.initAddr),reg,reg,reg);
