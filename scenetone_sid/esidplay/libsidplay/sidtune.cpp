...
static const char * const defaultFileNameExt[] = 
{
...
EXPORT_C sidTune::sidTune( const char* fileName )
{
	safeConstructor();
	if (fileName != 0)
	{
...
bool sidTune::setInfo(struct sidTuneInfo& /*inInfo*/)
	{
 ...
 *
 * @param outInfo sidTuneInfo where info will be written
 */
	{
	outInfo = info;
	}
...
udword sidTune::loadFile(const char* fileName, ubyte** bufferRef)
{
	...
	ELOG1(_L8("connecting to Fileserver..."));

	RFs fs;
	ret = fs.Connect();
	if(ret != KErrNone)
		{
		ELOG2(_L8("Error [ret=%d]\n"), ret);
		return 0;
		}
	else ELOG1(_L8("OK\n"));

	ELOG2(_L8("opening file %s..."), fileName);

	TBuf8<256> buf8((const TUint8*)fileName);
	TBuf<256> buf;
	buf.Copy(buf8);

	RFile file;
	ret = file.Open(fs, buf, EFileRead | EFileShareReadersOnly);
	if(ret != KErrNone)
#else
...
//	int ppRet = depp( myIn, bufferRef ); //TODO - use depp()
//		if ( ppRet == TRUE )
//		{
...
//		else if ( ppRet == FALSE )  // must be uncompressed file
		{
			...
			*bufferRef = new ubyte[fileLen+1];
			...
			{
#if defined (__SYMBIAN32__)
			TPtr8 ptr(*bufferRef + (fileLen - restFileLen), INT_MAX);
			ret = file.Read(ptr);
			if(ret != KErrNone)
				{
				ELOG2(_L8("ret = %d\n"), ret);
			...
			{
#if defined (__SYMBIAN32__)
			TPtr8 ptr(*bufferRef + (fileLen - restFileLen), restFileLen);
			ret = file.Read(ptr);
			if(ret != KErrNone)
				{
				ELOG2(_L8("ret = %d\n"), ret);
...
// ------------------------------------------ Looking for a description file.
				...
				const char * const* tmpFileNameExt = fileNameExtensions;
