...
struct emuConfig
{
	...
	int memoryMode;        ///< MPU_BANK_SWITCHING, MPU_TRANSPARENT_ROM,MPU_PLAYSID_ENVIRONMENT
...
#define numberOfC64addr  9 //TODO: sort this out
...
//	friend bool sidEmuInitializeSong(emuEngine &, sidTune &, uword songNum);
 ...
 protected:  // --------------------------------------------------- protected
  ...
  public:  // ------------------------------------------------------- public variables
	C6510* iThe6510;     ///< pointer to the 6510 which we own
    sidEmu* iTheSidEmu; ///< pointer to the sidEmu (6581) object
    Mixer* iTheMixer;
	sbyte* ampMod1x8;
	
//  private:  // ------------------------------------------------------- private variables
  public:  // TODO // ------------------------------------------------------- private variables
	emuConfig config;
	
#if defined(SIDEMU_TIME_COUNT)
	udword bytesCount;    ///< used for fading and duration
	int secondsTotal;     ///< listening mileage
	int secondsThisSong;  ///< listening mileage
#endif
	
	ubyte bufferScale;
	ubyte playRamRom;

	ubyte oldValues[numberOfC64addr];  ///< for the player
